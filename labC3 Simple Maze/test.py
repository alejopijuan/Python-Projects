...
class LabC3Tests(unittest.TestCase):
...
""")
  ...
  def test_maze1(self):
    ...
    f.close()
    self.assertListEqual(LabC3.solve(None, maze), \
        [(0, 0), (0, 1)], \
  ...
  def test_nomaze(self):
    ...
    maze=Maze.Maze([line for line in f], 0)
    f.close()
    self.assertIsNone(LabC3.solve(None, maze), \
...
""")
    enter()
    import LabC3
    self.assertGreater(len(LabC3.__doc__),20, msg=msg)
    exit()
